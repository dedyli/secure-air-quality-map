...
// To deploy this:
// 1. Place this file in a `functions` directory.
// 2. Create a `netlify.toml` file in your root directory.
// 3. Run `npm install express serverless-http node-fetch cors`.
// 4. Deploy to Netlify.
...
// Set up a router to handle all sub-paths
const router = express.Router();
router.get('/*', async (req, res) => {
    // The '*' captures the rest of the path (e.g., 'latest', 'measurements')
    ...
    try {
        ...
        res.status(500).json({ message: 'Error fetching data from the proxy server.', details: error.message });
    }
});

// Mount the router on the /api path
app.use('/.netlify/functions/openaq', router);
